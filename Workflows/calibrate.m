...
% 根据经验，仿真开始的一小段时间内求解尚未完全稳定，容易出现值突变，舍去
startIndex = 1001;
...
for apIndex = 1:length(accelPedalValues)
    for gear = gearNumbers
        ...
        simIn(index) = simIn(index).setBlockParameter( ...
            modelName + "/GearConstant", 'Value', num2str(gear));
        simIn(index) = simIn(index).setBlockParameter( ...
            modelName + "/AccelConstant", 'Value', ...
...
for apIndex = 1:length(accelPedalValues)
    for gear = gearNumbers
        ...
        smoothedVelocities = smooth(velocityTimeTable.Data, 'moving', 3);
        % 差分得到加速度
...
for apIndex = 1:length(accelPedalValues)
    ...
    figure("Name", num2str(accelPedalValues(apIndex))+"%")
    hold on
    ...
    xlabel("车速")
    ylabel("加速度")
    title("加速踏板开度："+num2str(accelPedalValues(apIndex))+"%")
